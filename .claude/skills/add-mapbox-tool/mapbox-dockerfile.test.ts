/**
 * Dependency guard for the Mapbox MCP server (host/vitest tree).
 *
 * `@mapbox/mcp-server` is a stdio CLI installed globally in the image,
...
describe('container/Dockerfile installs @mapbox/mcp-server', () => {
  const text = dockerfile();

  it('pins the version via an ARG', () => {
    expect(text).toMatch(/^\s*ARG\s+MAPBOX_MCP_VERSION=/m);
  });

  it('installs the package pinned to that ARG in a pnpm global-install block', () => {
    const installsMapbox = /pnpm\s+install\s+-g[\s\S]*?@mapbox\/mcp-server@\$\{MAPBOX_MCP_VERSION\}/.test(text);
    expect(installsMapbox).toBe(true);
  });
});
